...
| [AWS App Runner デプロイ](docs/apprunner.md) | AWS Console / AWS CLI からの App Runner デプロイ手順 |
...
リポジトリルートの `apprunner.yaml` で App Runner のビルド・起動設定を管理しています。AWS Console での作成手順、AWS CLI の `create-service` 入力 JSON、Secrets Manager / SSM Parameter Store を使った `MONGO_URI` の扱いは [AWS App Runner デプロイ](docs/apprunner.md) を参照してください。
